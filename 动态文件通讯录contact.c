...
{
	assert(pc);
	FILE* file = fopen("scontact.txt", "wb");
	if (file == NULL)
	{
		perror("savecontact");
		return;
	...
	for (int i = 0; i < pc->count; i++)
	{
		fwrite(pc->data + i, sizeof(peoinfo), 1, file);
	}
	fclose(file);
	file = NULL;
	printf("存档成功\n");
...
{
	assert(pc);
	FILE* file = fopen("scontact.txt", "rb");
	if (file == NULL)
	{
		perror("Ladecontact");
		return;
	...
	peoinfo qwq = { 0 };
	while (fread(&qwq, sizeof(peoinfo), 1, file) == 1)
	{
		kuorong(pc);
		pc->data[pc->count] = qwq;
		pc->count++;
	}
	fclose(file);
	file = NULL;
